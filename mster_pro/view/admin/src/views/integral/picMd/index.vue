  ...
  <div class="divBox">
    <el-card class="box-card">
      ...
      <el-table v-loading="listLoading" :data="tableData.data" style="width: 100%" size="mini">
        ...
        <el-table-column label="热卖图片" min-width="80" prop="pic">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 添加
      onAdd() {
        this.$modalForm(picMdCreateApi()).then(() => this.getList());
      },
      // 编辑
      onEdit(id) {
        this.$modalForm(picMdUpdateApi(id)).then(() => this.getList())
      },
      // 删除
      handleDelete(id, idx) {
        this.$modalSure().then(() => {
          picMdDeleteApi(id).then(({message}) => {
      ...
      //是否显示
      onchangeIsShow(row) {
        picMdStatusApi(row.id, row.status).then(({message}) => {
          this.$message.success(message)
        }).catch(({message}) => {
          this.$message.error(message)
        })
