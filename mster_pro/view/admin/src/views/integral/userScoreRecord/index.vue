  ...
  <div class="divBox">
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <div class="container">
          <el-form :inline="true">
            ...
            <el-form-item label="积分用户：" class="mr10">
              <el-input v-model="tableFrom.nickname"   placeholder="请输入积分用户" class="selWidth"  size="small">
                <el-button slot="append" icon="el-icon-search" size="small" @click="getList(1)"/>
              </el-input>
            </el-form-item>
            <el-form-item label="类型：" class="mr10">
              <el-select v-model="tableFrom.type" clearable  placeholder="请选择类型" class="selWidth" @change="getList(1)">
                <el-option label="支出" value="1"/>
                <el-option label="收入" value="2"/>
              </el-select>
            </el-form-item>
            <el-form-item label="备注：" class="mr10">
              <el-input v-model="tableFrom.remark"   placeholder="请输入备注" class="selWidth"  size="small">
                <el-button slot="append" icon="el-icon-search" size="small" @click="getList(1)"/>
              </el-input>
            </el-form-item>
          </el-form>
      ...
      <el-table v-loading="listLoading" :data="tableData.data" style="width: 100%" size="mini">
        ...
        <el-table-column prop="score" label="实际支付积分" sortable min-width="60"/>
        <el-table-column label="类型" min-width="100">
          <template slot-scope="{row}">
            <span>{{ row.type === 1 ? '支出' : '收入' }}</span>
          </template>
        </el-table-column>
        <el-table-column label="备注" min-width="300">
          <template slot-scope="scope">
            <div class="mb5 content_font">{{ scope.row.remark }}</div>
          </template>
        </el-table-column>
...
<script>
  import {
    userScoreRecordListApi,
  } from "@/api/userScoreRecord";
  ...
  export default {
    ...
    methods: {
      ...
      getList(num) {
        ...
        userScoreRecordListApi(this.tableFrom)
          .then((res) => {
      ...
      // handleDelete(id, idx) {
      //   this.$modalSure().then(() => {
      //     merchantsDeleteApi(id).then(({message}) => {
