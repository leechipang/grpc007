  ...
  <div class="divBox">
    <el-card class="box-card">
          ...
          <div class="demo-input-suffix acea-row">
            <el-form inline size="small">
              <el-form-item label="商户分类：">
                <el-cascader v-model="tableFrom.cate_id" class="selWidth" :options="merCateList" :props="props"
                             clearable @change="getList(1)"/>
              </el-form-item>
      ...
      <el-table v-loading="listLoading" :data="tableData.data" style="width: 100%" size="mini"
          ...
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              ...
              <el-form-item label="积分：">
                <span>{{ props.row.score | filterEmpty }}</span>
              </el-form-item>
        ...
        <el-table-column prop="score" label="商品积分" min-width="90"/>
        ...
        <el-table-column prop="status" label="是否显示" min-width="100">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.is_used" :active-value="1" :inactive-value="0" active-text="显示"
                       inactive-text="隐藏" @change="onchangeIsShow(scope.row)"/>
          </template>
        </el-table-column>
        <el-table-column prop="create_time" label="创建时间" min-width="150"/>
        <el-table-column v-if="Number(tableFrom.type) < 7" key="8" label="操作" min-width="100" fixed="right"
                         align="center">
          <template slot-scope="scope">
            <el-button v-if="Number(tableFrom.type) < 7" type="text" size="small" @click="onEdit(scope.row.product_id)">
              编辑
            </el-button>
            <el-button v-if="tableFrom.type === '6' || tableFrom.type === '1'" type="text" size="small"
                       @click="toVirtualSales(scope.row.product_id)">虚拟销量
            </el-button>
            <el-button v-if="tableFrom.type === '6'" type="text" size="small" @click="toExamine(scope.row.product_id)">
              审核
            </el-button>
            <el-button v-if="Number(tableFrom.type) < 3" type="text" size="small"
                       @click="toOff([scope.row.product_id])">强制下架
            </el-button>
          </template>
        </el-table-column>
      </el-table>
