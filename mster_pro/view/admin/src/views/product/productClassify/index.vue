  ...
  <div class="divBox">
    <el-card class="box-card">
      ...
      >
        ...
        <el-table-column label="分类背景图" min-width="80">
          <template slot-scope="scope">
            <div class="demo-image__preview">
              <el-image
                style="width: 36px; height: 36px"
                :src="scope.row.backImg?scope.row.backImg:moren"
                :preview-src-list="[scope.row.backImg?scope.row.backImg:moren]"
              />
            </div>
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="scope">
            <el-switch
              v-model="scope.row.in_home"
              :active-value="1"
              :inactive-value="0"
              active-text="显示"
              inactive-text="隐藏"
              @change="onchangeIsShows(scope.row)"
            />
          </template>
        </el-table-column>
        <el-table-column
...
<script>
import {
  storeCategoryListApi, storeCategoryCreateApi, storeCategoryUpdateApi, storeCategoryDeleteApi, storeCategoryStatusApi,storeCategoryStatusApis
} from '@/api/product'
export default {
  ...
  methods: {
    ...
    onchangeIsShows(row) {
      storeCategoryStatusApis(row.store_category_id, row.in_home).then(({ message }) => {
        this.$message.success(message)
      }).catch(({ message }) => {
        this.$message.error(message)
      })
