			 ...
			 @scroll="scroll">
				<view id="past0">
					...
					<view class='wrapper'>
						...
						<view class='introduce'><text v-if="storeInfo.merchant.is_trader" class="font-bg-red bg">自营</text>{{storeInfo.store_name ? storeInfo.store_name : ''}}</view>
						...
						<view class='coupon acea-row row-between-wrapper' @click='couponTap' style="margin-top: 20rpx;" v-if="coupon.list.length">
							<view class='hide line1 acea-row'>
								优惠券：
				...
				<view class="store-wrapper" v-if="storeInfo.merchant && hide_mer_status !=1">
					<view class="store-hd">
						<view class="store-info">
							...
							<view class="info">
								<view class="name">{{storeInfo.merchant.mer_name ? storeInfo.merchant.mer_name : ''}}<text v-if="storeInfo.merchant.is_trader"
				...
				<view class='product-intro' id="past3">
					...
					<view class='conter' v-if="description">
						<jyf-parser :domain='domain' :html="description.content.replace(/<br\/>/ig, '')" ref="article" :tag-style="tagStyle"></jyf-parser>
					</view>
				</view>
		...
		<view class='footer acea-row row-between-wrapper'>
			...
			<view v-else class='bnt acea-row'>
				<form report-submit='true'>
					<button class='buy bnts sold_out' form-type="submit" disabled>商品已售罄</button>
				</form>
			</view>
		</view>
