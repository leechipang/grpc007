  ...
  <div class="divBox">
    <el-card class="box-card">
      ...
      >
        ...
        <el-table-column prop="score" label="退款积分" min-width="130" />
        <el-table-column prop="nickname" label="商品信息" min-width="330">
          <template slot-scope="scope">
            ...
            >
              ...
              <span v-if="scope.row.order.activity_type === 2 && val.product.cart_info.productPresellAttr"
                class="tabBox_pice"
              >{{ '￥'+ val.product.cart_info.productPresellAttr.presell_price + ' x '+ val.refund_num }}</span>
              <span v-else-if="scope.row.order.activity_type === 3 && val.product.cart_info.productAssistAttr.assist_price"
                    class="tabBox_pice"
              >{{ '￥'+ val.product.cart_info.productAssistAttr.assist_price + ' x '+ val.refund_num }}</span>
              <span v-else
                class="tabBox_pice"
              >{{ '￥'+ val.product.cart_info.productAttr.price + ' x '+ val.refund_num }}</span>
            </div>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getPresellTotal(row){
      let sum = 0
      for (let i = 0; i < row.length; i++) {
        sum += row[i].product.cart_info.productPresellAttr.presell_price * row[i].refund_num
      }
      return sum
    },
    //助力商品退款金额
    getAssistTotal(row){
      let sum = 0
      for (let i = 0; i < row.length; i++) {
        sum += row[i].product.cart_info.productAssistAttr.assist_price * row[i].refund_num
      }
      return sum
    },
    // 详情
