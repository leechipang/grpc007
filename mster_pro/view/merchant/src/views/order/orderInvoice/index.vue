      ...
      >
        ...
        <el-table-column label="订单状态" min-width="90">
          <template slot-scope="scope">
            <span v-if="scope.row.storeOrder.paid === 0 && scope.row.storeOrder.status === 0">待付款</span>
            <span v-else>{{ scope.row.storeOrder.status | orderStatusFilter }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="发票联系人" min-width="100">
          <template slot-scope="scope">
            <span>{{ scope.row.storeOrder.real_name }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="storeOrder.mark" label="发票备注" min-width="120"/>
        <el-table-column label="操作" min-width="180" fixed="right" align="center">
          <template slot-scope="scope">
            <el-button v-if="scope.row.status === 0" type="text" size="small" @click="getInvoiceInfo(scope.row.order_receipt_id,'invoice')">开票</el-button>
...
export default {
  ...
  methods: {
    ...
    getInvoiceInfo(id,type){
      this.id = id;
      this.type = type;
      let ids = id ? id.toString() : this.getInvoic();
      let params = {ids: ids}
      type === 'invoice' ?
    ...
    // 开票
    handleInvoic(){
        let ids = this.id ? this.id.toString() : this.getInvoic();
        let params = {
          ids: ids,
