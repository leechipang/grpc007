  ...
  <div class="divBox">
    <el-card class="box-card">
      ...
      <el-table v-loading="listLoading" :data="tableData.data" style="width: 100%" size="mini"  @rowclick.stop="closeEdit" :row-class-name="tableRowClassName">
        ...
        <el-table-column prop="product.price" label="原价" min-width="90" />
        ...
        <el-table-column prop="sales" label="已售商品数" min-width="80" />
        <el-table-column label="拼团活动状态" min-width="90">
          <template slot-scope="scope">
        ...
        <el-table-column label="活动时间" min-width="200">
          <template slot-scope="scope">
            <div>开始日期：{{ scope.row.start_time && scope.row.start_time ? scope.row.start_time : "" }}</div>
            <div>结束日期：{{ scope.row.end_time && scope.row.end_time ? scope.row.end_time : "" }}</div>
          </template>
        </el-table-column>
        
        <el-table-column prop="stock" label="限量" align="center" min-width="80" />
        <el-table-column prop="product.sort" align="center" label="排序" min-width="80">
            <template slot-scope="scope">
                <span v-if="scope.row.index === tabClickIndex">
                    <el-input v-model="scope.row['product']['sort']" maxlength="300" size="mini" @blur="inputBlur(scope)" autofocus/>
                </span>
      ...
      <div v-loading="loading" style="margin-top: 5px;">
        <div class="box-container">
          ...
          <div class="acea-row">
            ...
            <div class="list sp"><label class="name">限量：</label>{{ formValidate.stock }}</div>
