                        ...
                        <el-form-item v-if="formValidate.spec_type === 0">
                            <el-table :data="OneattrValue" border class="tabNumWidth" size="mini">
                                ...
                                <el-table-column align="center" label="助力价" min-width="80">
                                    <template slot-scope="scope">
                                        <el-input v-model.number="scope.row['assist_price']" type="number" class="priceBox" :min="0" :max="scope.row['price']" @blur="limitPrice(scope.row)"/>
                                    </template>
                                </el-table-column>
                ...
                <el-row :gutter="24">
                    <el-form-item v-if="formValidate.spec_type === 1" class="labeltop" label="规格列表：">
                        <el-table ref="multipleTable" :data="ManyAttrValue" tooltip-effect="dark" highlight-current-row :row-key="(row) => { return row.id }">
                            ...
                            <el-table-column align="center" label="助力价" min-width="80">
                                <template slot-scope="scope">
                                    <el-input v-model.number=" scope.row['assist_price']" type="number" class="priceBox" :min="0" :max="scope.row['price']" />
                                </template>
                            </el-table-column>
                            <el-table-column align="center" label="成本价" min-width="80">
                                <template slot-scope="scope">
        ...
        getInfo(id) {
            ...
            this.$route.params.id ? assistProDetailApi(id).then(async (res) => {
                    ...
                    if (this.formValidate.spec_type === 0) {
                        ...
                        //   this.$set(this.OneattrValue[i], 'assist_price', val.assistSku.assist_price ? val.assistSku.assist_price : 0)
                        // });
                        this.$set(this.OneattrValue[0], 'assist_price', this.OneattrValue[0].assistSku.assist_price ? this.OneattrValue[0].assistSku.assist_price : 0)
