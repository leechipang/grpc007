	...
	<view class="product-con">
				...
				<view id="past0">
					...
					<view class='wrapper'>
						...
						<view class='introduce'>
							<text v-if="(storeInfo.merchant && storeInfo.merchant.is_trader)" class="font-bg-red bg">自营</text>{{storeInfo.store_name}}
						</view>
						<view class='label acea-row row-between-wrapper'>
							<view>类型:{{combinationInfo.buying_count_num ? combinationInfo.buying_count_num : 0}}人团</view>
							<view>库存:{{combinationInfo.stock ? combinationInfo.stock : 0}}</view>
							<view>已拼:{{combinationInfo.sales ? combinationInfo.sales : 0}}{{storeInfo.unit_name ? storeInfo.unit_name : ''}}</view>
						</view>
						...
						<view class='assemble'>
							<view class='item acea-row row-between-wrapper' v-for='(item,index) in pink' :key='index' v-if="index < AllIndex && pink.length>0">
								<view class='pictxt acea-row row-between-wrapper'>
									<view class='pictrue'>
										<image :src='item.initiator.avatar'></image>
									</view>
				...
				<view v-if="hide_mer_status == 0 && storeInfo.merchant" class="store-wrapper">
					<view class="store-hd">
						<view class="store-info">
							...
							<view class="info">
								<view class="name">{{storeInfo.merchant.mer_name}}<text v-if="(storeInfo.merchant && storeInfo.merchant.is_trader)"
									 class="font-bg-red ml8">自营</text></view>
		...
		<view class='footer_count'>
			...
			<view class="acea-row row-between-wrapper footer">
				<navigator hover-class="none" class="item" :url="`/pages/chat/customer_list/chat?mer_id=${storeInfo.mer_id}&uid=${uid}&productId=${storeInfo.product_id}&combinationId=${id}`">
				...
				<view class='bnt acea-row'>
					<!-- <form @submit="goBuy" report-submit='true' v-if="attr.productSelect">
						...
						<button v-else class='buy bnts' form-type="submit">立即开团</button>
					</form>			 -->
					<form @submit="joinCart" report-submit='true'>
						<button class='joinCart bnts' @tap="openAlone">单独购买</button>
					</form>
					<form @submit="goBuy" report-submit='true' v-if="attr.productSelect ">
		...
		methods: {
			...
			setCollect: function() {
				if (this.isLogin === false) {
					...
					if (this.storeInfo.isRelation) {
						collectDel({
							type: 1,
							type_id: this.combinationInfo.product_id
						}).then(res => {
			...
			downloadFilePromotionCode: function(successFn) {
				...
				getProductCode(that.id, {
					...
					this.codeImg = 'https://cremb.oss-cn-beijing.aliyuncs.com/315ab7eeaa71000275b692d545b868cd.jpg';
					// that.$util.Tips({
					// 	title: err
					// });
					// that.posters = false;
...
<style scoped lang="scss">
	...
	// .product-con .combination_wrapper{
	// 	background: #fff;
	// }
