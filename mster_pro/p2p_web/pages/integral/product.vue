  ...
  <view class="product-con">
      ...
      >
          ...
          <view class="wrapper">
            <view class="share acea-row row-between row-bottom">
              <view class="money font-color">
                <view class="p-name clear">
                  ...
                  <text>{{ storeInfo.score }}</text>
                  <text v-if="storeInfo.price != '0.00'">+</text>
                  <text v-if="storeInfo.price != '0.00'"
              ...
              <view class="p-shar" @click="listenerActionSheet">
                <image src="/static/img/17.png"></image>
              </view>
            </view>
            <view class="introduce">
              <text v-if="storeInfo.merchant.is_trader" class="font-bg-red bg"
                >自营</text
              >
              {{ storeInfo.store_name ? storeInfo.store_name : "" }}</view
            >
            <view
            ...
            >
              ...
              <view
                ...
                }}{{ storeInfo.unit_name ? storeInfo.unit_name : "" }}</view
              >
              <view
          ...
          <view class="store-hd">
            <view class="store-info">
              ...
              <view class="info">
                <view class="name"
                  >{{
                    ...
                    v-if="storeInfo.merchant.is_trader"
                    class="font-bg-red ml8"
                    >自营</text
                  ></view
                >
                <view class="txt"
              ...
              <navigator
                ...
                :url="'/pages/store/home/index?id=' + storeInfo.merchant.mer_id"
                class="link"
                hover-class="none"
                >进店</navigator
              >
            ...
            <view class="img-box">
              ...
              >
                ...
                <view class="txt">
                  ...
                  <view class="price p-jifen">
                    <image
                      class="fen"
                      v-if="item.score > 0"
                      src="/static/img/20.png"
                    ></image>
                    <text class="p-ji" v-if="item.score > 0">{{
                      item.score
                    }}</text>
                    <text
                      class="p-add"
                      v-if="item.score > 0 && item.price != '0.00'"
                      >+</text
                    >
        ...
        <view class="product-intro" id="past3">
          ...
          <view class="conter" v-if="description">
            <jyf-parser
              :domain="domain"
              :html="description.content.replace(/<br\/>/gi, '')"
              ref="article"
              :tag-style="tagStyle"
            ></jyf-parser>
          </view>
    ...
    <view class="footer acea-row row-between-wrapper">
      <view class="item" @click="couponTap2">
        <view class="i-kefu">
          <image src="/static/img/15.png"></image>
        </view>
        <view>客服</view>
      </view>
      <!-- <view @click="go('/pages/integral/submit?cartId=66')" class="duihuan"> -->
      <view @click="goBuy" class="duihuan"> 立即兑换 </view>
      ...
      >
        <view class="iconfont icon-gouwuche1">
          <text v-if="CartCount > 0" class="num bg-color">{{
            CartCount || 0
          }}</text>
        </view>
        <view>购物车</view>
      ...
      <view v-else class="bnt acea-row">
        <form report-submit="true">
          <button class="buy bnts sold_out" form-type="submit" disabled>
            商品已售罄
          </button>
        </form>
      </view> -->
...
import { getUserInfo, userShare, imgToBase } from "@/api/user.js";
...
import productWindow from "@/components/productWindow2";
...
export default {
  ...
  methods: {
    ...
    goProDetail(item) {
      if (item.score > 0) {
        uni.navigateTo({
          url: "/pages/integral/product?id=" + item.product_id,
        });
      } else {
        uni.navigateTo({
    ...
    getUserInfo: function () {
      ...
      that.$set(
        that.sharePacket,
        "isState",
        that.sharePacket.priceName != 0 ? false : true
      );
    ...
    ChangeAttr: function (res) {
      ...
      if (productSelect && productSelect.stock > 0) {
        this.$set(
          ...
          "image",
          productSelect.image ? productSelect.image : this.storeInfo.image
        );
        this.$set(this.attr.productSelect, "price", productSelect.price);
        this.$set(this.attr.productSelect, "score", productSelect.score);
      ...
      getProductDetail(that.id)
        .then((res) => {
          ...
          if (storeInfo.video_link && storeInfo.slider_image.length > 1) {
            ...
            storeInfo.slider_image.length === 1
          ) {
            this.videoCoverImg = storeInfo.slider_image[0];
          }
          ...
          that.$set(
            that,
            "reply",
            res.data.topReply ? [res.data.topReply] : []
          );
    ...
    DefaultSelect: function () {
      this.$set(this.attr.productSelect, "score", this.storeInfo.score);
      ...
      postCartAdd(q)
        .then(function (res) {
          ...
          if (news) {
            uni.navigateTo({
              url:
                // "/pages/users/order_confirm/index?cartId=" + res.data.cart_id,
                "/pages/integral/submit?cartId=" + res.data.cart_id,
            });
          } else {
            that.$util.Tips({
              title: "添加购物车成功",
    ...
    async goPoster() {
      ...
      arr2 = [
        that.posterbackgd,
        await this.fileStoreImage(this.storeImage),
        await this.fileStoreImage(this.codeImg),
      ];
      // #endif
      // #ifdef H5
      arr2 = [
        that.posterbackgd,
        await this.imgToBase(that.storeImage),
        await this.imgToBase(this.codeImg),
      ];
      // #endif
      //生成推广海报
