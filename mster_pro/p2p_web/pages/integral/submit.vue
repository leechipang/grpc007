<template>
	<view>
		<uni-popup ref="popp" type="center" class="pop">
			<view class="sel-jf">
				<text class="s-tit">选择积分</text>
				<ul class="s-list">
					<li  @click="exchange(index,item)" v-for="(item,index) in list1" :key=index>
						<image :src='item.merchant.logo'></image>
						<text>{{item.merchant.name | string(10)}}</text>
						<image v-if="exIndex == index" src='/static/img/16.png'></image>
					</li>
				</ul>
				<view @click="close" class="s-ok">确认选择</view>
			</view>
		</uni-popup>
		...
		<view class='order-submission'>
				...
				<view class="store-item" v-for="(item,index) in cartInfo" :key="index">
						...
						<view v-if="goods.product_type == 2">
							<view class="product-item">
								...
								<view class="content event_content">
									<view class="name line1"><text class="event_name event_bg">预售</text>{{goods.productPresell.store_name}}</view>
							...
							<view v-if="goods.productPresell.presell_type === 2" class="event_payTime">
								<view class="event_progress">
									<view class="progress_step">
										...
										<text class="price color_red">￥{{ (goods.productPresellAttr.down_price * goods.cart_num).toFixed(2) }}</text>
									</view>
									<view class="progress_step">
										<text class="name">尾款</text>
										<text class="price">￥{{ (goods.productPresellAttr.final_price * goods.cart_num).toFixed(2) }}</text>
									</view>
									<view class="progress_pay">{{ goods.productPresell.final_start_time | filterDay}}开始支付尾款</view>
								</view>
							</view>
						</view>
						<view v-else>
							<view class="product-item">
								...
								<view class="content">
									...
									<view class="label">{{goods.productAttr.sku}} X{{goods.cart_num}}</view>
									<view class="price">
										...
										<block v-else>
											<view class="p-name clear">
												<image src="/static/img/20.png"></image>
												<text>{{ goods.productAttr.score }}+</text>
					...
					<view class="wrapper">
						<view class="boxs" v-if="item.take.mer_take_status != 0">
							...
							<view class="store-address" v-if="item.isTake">
						...
						<view class='item acea-row row-between-wrapper' v-if="!pinkId && !BargainId && !combinationId && !seckillId && order_type != 3 && order_type != 4">
							<view>优惠券</view>
            ...
            <!-- 兑换方式 -->
            <view class="dh-type" v-if="list1.length > 0">
                <text class="tit">兑换方式</text>
                <ul>
                    <li @click="open" v-for="(item,index) in 1" :key="index" class="clear">
                        <view class="pay">
							<image src='/static/img/20.png'></image>
                        </view>
                        <view class="fen">
                            <view class="yong">
                                <text>可用积分</text>
                                <text>{{duserScore}}积分</text>
                            </view>
                            <view class="tao">
                                <text>积分不足，请选择第三方积分</text>
                                <!-- <text>{{item.merchant.rate_to_us}}{{item.merchant.name  | string(5)}}=1积分</text> -->
								<text style="color:#CC0000;font-weight:bold">去选择</text>
                            </view>
                        </view>
                        <!-- <view v-if="exIndex == index" class="ok">
                            <image src='/static/img/16.png'></image>
                        </view> -->
                    </li>
                    <!-- <li class="clear">
                        <view class="pay">
                            <image src='/static/img/13.png'></image>
                        </view>
                        <view class="fen">
                            <view class="yong">
                                <text>可用积分</text>
			...
			<view class='footer acea-row row-between-wrapper'>
				...
				<view class='settlement' :class='addressInfo.real_name ? "" : "disabled"' style='z-index:100' @tap="SubOrder">{{addressInfo.real_name ? '立即兑换':'选择地址'}}</view>
			</view>
		</view>
...
<script>
	...
	import { userScore,ubind } from "@/api/api.js";
	import {
		orderConfirm,
	...
	export default {
		...
		methods: {
			...
			close(){
				exchange({
					mer_id:this.list1[this.exIndex].mer_id,
					cart_id:this.cartId.split(",")[0],
				}).then(res => {
					this.$util.Tips({
						title: '兑换成功'
					})
				...
				this.$refs.popp.close()
				// console.log(this.cartId.split(",")[0])
			...
			getConfirm: function(address_id) {
				...
				orderConfirm({
					...
					this.duserScore = res.data.userScore
					let conponNum = 0
					// 默认选中
			...
			onAddress: function() {
				...
				that.pagesUrl = '/pages/users/user_address_list/index?type=2&cartId=' + this.cartId + '&pinkId=' + this.pinkId +
					'&couponId=' + this.couponId;
			...
			SubOrder2: function(e) {
				...
				data = {
					...
					user_score_id:this.list1.length > 0?this.list1[this.exIndex].id:undefined,
					// #ifdef H5
...
.p-name {
  ...
  text {
	...
	.order-submission .footer .settlement {
        ...
        background: linear-gradient(270deg,#e22c1e, #f86618);
		border-radius: 50rpx;
		text-align: center;
		line-height: 82rpx;
