<template>
  <view
    :style="{
      'background-image': `linear-gradient(0deg, rgba(0, 0, 0, 0.2) 0%, rgba(0, 0, 0, 0.8) 40%),url(${store.mer_banner})`,
    }"
    class="store-home"
  >
    <!-- 搜索 -->
    ...
    <view
      :style="{
        ...
        'padding-right': `${
          (systemInfo.windowWidth - menuButtonInfo.left) * 2 + 34
        }rpx`,
      }"
      class="header"
    >
      ...
      ></view>
      <navigator
        :url="'/pages/store/list/index?mer_id=' + id"
        hover-class="none"
        class="search"
    ...
    <view v-show="navShow && tabActive === 0" class="nav">
      <view class="nav-cont">
          ...
          <view class="cont">
            ...
            <text
              :class="[
                'arrow-icon',
                'iconfont',
                select.show ? 'icon-xiangshang' : 'icon-xiangxia',
              ]"
            ></text>
          </view>
        </view>
        ...
        >
          <view class="cont">
            ...
            <image
              :src="
                sortPrice ? '/static/images/up.png' : '/static/images/down.png'
              "
            ></image>
          </view>
        </view>
          ...
          <view class="cont">
            <text
              :class="[
                'layout-icon',
                'iconfont',
                isColumn ? 'icon-pailie' : 'icon-tupianpailie',
              ]"
            ></text>
          </view>
        </view>
      ...
      <view v-show="select.show && navShow" class="select">
        <view
          ...
          :key="item.id"
          :class="{ active: item.id === select.selected }"
          class="item"
        ...
        <view class="nav-cont">
          ...
          >
            <view class="cont">
              价格
              <image
                v-if="navActive === 2 && where.order == 'price_asc'"
                src="/static/images/up.png"
              ></image>
              <image
                v-if="navActive === 2 && where.order == 'price_desc'"
      ...
      <view class="tab-cont">
        ...
        <view v-show="tabActive === 0">
          ...
          <view class="goods-wrap">
            ...
            >
              ...
              >
                ...
                <view class="text">
                  ...
                  <view class="score"
                    >{{ item.rate }}评分 {{ item.reply_count }}条评论</view
                  >
                </view>
                <view
                  v-if="
                    item.max_extension &&
                    (item.product_type == 0 || item.product_type == 2)
                  "
                  class="foot"
                >
            ...
            <emptyPage
              v-if="goods.length == 0 && !goodsLoading"
              title="暂无商品~"
            ></emptyPage>
          </view>
        </view>
        ...
        <view v-show="tabActive === 1">
          <view class="category">
            ...
            >
              <view
                class="head"
                @click="goCategoryGoods(item.store_category_id)"
              >
              ...
              <view v-if="item.children" class="body">
                <view
                  ...
                  :key="value.store_category_id"
                  class="item"
                  @click="goCategoryGoods(value.store_category_id)"
                  >{{ value.cate_name }}</view
                >
              </view>
            </view>
          </view>
              ...
              <view class="right">
                ...
                <view class="time-wrap" style="justify-content: space-between">
                  <block v-if="item.coupon_type == 1">
                    <view class="time"
                      >{{ item.use_start_time | dateFormat }}-{{
                        item.use_end_time | dateFormat
                      }}</view
                    >
                  </block>
  ...
  methods: {
    ...
    receiveCoupon(item) {
      ...
      if (that.isLogin === false) {
        ...
        setCouponReceive(item.coupon_id)
          .then((res) => {
            item.issue = 1;
            uni.showToast({
              title: res.message,
    ...
    getGoods: function () {
      ...
      that.loadTitle = "";
      getStoreGoods(that.id, that.where)
        .then((res) => {
  ...
  .text {
    ...
    .score {
      ...
      .star {
        ...
        height: 19rpx;
        margin-right: 10rpx;
        background: url(../../columnGoods/images/star.png) left top/100% 100%
          no-repeat;
        overflow: hidden;
        ...
        view {
          ...
          width: 100%;
          height: 100%;
          background: url(../../columnGoods/images/star_active.png) left
            top/111rpx 19rpx no-repeat;
        }
      }
    }
  }
...
.nav {
  ...
  .select {
    ...
    .active {
      background: url(../../../static/images/active.png) right center/20rpx
        no-repeat;
      color: #e93323;
    }
  }
    ...
    .text {
      ...
      .money-wrap {
        ...
        .ticket {
          ...
          margin-top: 17rpx;
          margin-left: 0;
          background: url(../../../static/images/yh.png) top left/100% 100%
            no-repeat;
          line-height: 28rpx;
        }
      }
