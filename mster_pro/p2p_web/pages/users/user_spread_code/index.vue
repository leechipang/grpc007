	...
	export default {
		...
		methods: {
			...
			async spreadMsg() {
				...
				this.spreadData = res.data.poster
				this.nickName = res.data.nickname
				this.siteName = res.data.site_name

				let codeUrl = "?spread=" + this.userInfo.uid
				// #ifdef MP
				let mpUrl = await this.downloadFilestoreImage(res.data.qrcode)
				// #endif
				uni.showLoading({
					title: '海报生成中',
					mask: true
				...
				for (let i = 0; i < res.data.poster.length; i++) {
					...
					let arr2
					// #ifdef MP	
					arr2 = [mpUrl, await this.downloadFilestoreImage(res.data.poster[i].pic)]
					// #endif
					// #ifdef H5
					let img = await this.imgToBase(res.data.poster[i].pic)
					arr2 = [res.data.qrcode, img]
					// #endif
					that.$util.userPosterCanvas(arr2, res.data.nickname, res.data.mark, i, this.wd, this.hg, (tempFilePath) => {
