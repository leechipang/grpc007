<template>
  <view class="page-wrapper history">
    <block v-if="list.length > 0">
      <view
        class="item area-row"
        :class="{ gary: item.product.is_show == 1 && item.product.status == 1 }"
      ...
      >
        <view
          v-if="isEdit"
          class="checkbox"
          @click="historyAllCheck(item, index)"
        >
        ...
        <view class="item item_count" @click="goPage(item)">
          ...
          <view class="info">
            ...
            <view class="msg">
              <block
                v-if="item.product.is_show == 1 && item.product.status == 1"
              >
                <view class="price p-jifen">
                  <image
                    class="fen"
                    v-if="item.product.score > 0"
                    src="/static/img/20.png"
                  ></image>
                  <text class="p-ji" v-if="item.product.score > 0">{{
                    item.product.score
                  }}</text>
                  <text
                    class="p-add"
                    v-if="
                      item.product.score > 0 && item.product.price != '0.00'
                    "
                    >+</text
                  >
                  <text class="p-pri" v-if="item.product.price != '0.00'"
              ...
              <block v-else>
                ...
                <view class="btn" @click.stop="bindDelete(item, index)"
                  >删除</view
                >
              </block>
            </view>
    ...
    <view
      class="footer acea-row row-between-wrapper area-edit"
      v-if="list.length > 0"
    >
...
export default {
  ...
  methods: {
    ...
    bindDelete(item, index) {
      historyDelete(item.user_visit_id)
        .then((res) => {
    ...
    goPage(item) {
      if (item.product.score > 0) {
        uni.navigateTo({
          url: "/pages/integral/product?id=" + item.product.product_id,
        });
      } else {
        uni.navigateTo({
