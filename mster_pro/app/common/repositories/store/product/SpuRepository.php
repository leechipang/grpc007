...
use app\common\repositories\p2p\CategoryProductRepository;
...
class SpuRepository extends BaseRepository
{
    ...
    public $productFiled = 'S.product_id,S.store_name,S.image,activity_id,S.keyword,S.price,S.mer_id,spu_id,S.status,store_info,brand_id,cate_id,unit_name,S.rank,P.sort,P.score,sales,S.product_type,rate,reply_count,extension_type';
    ...
    public function getApiSearch($where, $page, $limit, $userInfo)
    {
        ...
        $category = app()->make(CategoryProductRepository::class);
        $categoryInfo = null;
        if ((isset($where['pid']) && $where['pid'] !== '') || (isset($where['ppid']) && $where['ppid'] !== '')) {
            $categoryInfo = $category->cateInfo($where['pid'] ?? $where['ppid']);
        }
        $count = $query->count();
        ...
        $list->append($append);
        return compact('count', 'categoryInfo', 'list');
    }
