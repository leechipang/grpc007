...
class ProductRepository extends BaseRepository
{
    ...
    protected $dao;
    protected $filed = 'product_id,mer_id,brand_id,unit_name,mer_status,rate,reply_count,store_info,cate_id,image,slider_image,store_name,keyword,sort,rank,is_show,sales,price,extension_type,refusal,cost,ot_price,score,stock,is_gift_bag,care_count,status,is_used,create_time,product_type,old_product_id';
    ...
    public function save($id, $settleParams, $content, $data = [])
    {
        ...
        if (isset($settleParams['attrValue'])) {
            $arr = array_chunk($settleParams['attrValue'], 31);
            foreach ($arr as $item) {
        ...
        if(isset($settleParams['data'])){
            ...
            $data['score'] = $settleParams['data']['score'];//积分
    ...
    public function setAttrValue(array $data, int $productId, int $productType, int $isUpdate = 0)
    {
        ...
        $pric = $stock = $ot_price = $score = $cost = 0;
        foreach ($data as $value) {
            $sku = '';
            ...
            $result['attrValue'][] = [
                ...
                "score" => $value['score'] ? (($value['score'] < 0) ? 0 : $value['score']) : 0,
            ...
            $score = ($score > $value['score']) ? $value['score'] : $score;
            $pric = ($pric > $value['price']) ? $value['price'] : $pric;
            $cost = ($cost > $value['cost']) ? $value['cost'] : $cost;

            $stock = $stock + intval($value['stock']);
        }
        $result['data'] = ['price' => $pric, 'stock' => $stock, 'ot_price' => $ot_price, 'score' => $score, 'cost' => $cost];
        return $result;
    }
...
//                $where = ['is_show' => 1, 'status' => 1];
    ...
    {
        $field = 'is_show,product_id,mer_id,image,slider_image,store_name,store_info,unit_name,price,cost,ot_price,stock,sales,video_link,product_type,extension_type,old_product_id,score';
        $with = [
            'attr',
            'content',
            'attrValue',
        ...
        if ($res){
//            if ($res['price'] > 0 && $res['score'] > 0) {
//                $res['price'] = $res['price'] . '+' . $res['score'] . '积分';
//            }elseif($res['price'] <= 0 && $res['score'] > 0){
//                $res['price'] = $res['score'] . '积分';
//            }
//            if ($res['merchant']['recommend']) {
//                foreach ($res['merchant']['recommend'] as $item) {
//                    if ($item['price'] > 0 && $item['score'] > 0) {
//                        $item['price'] = $item['price'] . '+' . $item['score'] . '积分';
//                    }elseif($item['price'] <= 0 && $item['score'] > 0) {
//                        $item['price'] = $item['score'] . '积分';
//                    }
//                }
//            }
...
//            if ($value['price'] > 0 && $value['score'] > 0) {
//                $_value['price'] = $value['price'] . '+' . $value['score'] . '积分';
//            }elseif($value['price'] <= 0 && $value['score'] > 0){
//                $_value['price'] = $value['score'] . '积分';
//            }
//            //秒杀
