...
class StoreCartRepository extends BaseRepository
{
    ...
    public function checkCartList($res, $hasCoupon = 0)
    {
        ...
        foreach ($res as $item) {
//            if ($item['productAttr']['price'] > 0 && $item['productAttr']['score'] > 0) {
//                $item['productAttr']['price'] = $item['productAttr']['price'] . '+' . $item['productAttr']['score'] . '积分';
//            }elseif($item['product']['price'] <= 0 && $item['product']['score'] > 0){
//                $item['productAttr']['price'] = $item['productAttr']['score'] . '积分';
//            }
     ...
     */
    public function getOneCart($id,$uid){
        $where = [$this->dao->getPk() => $id,'is_del'=>0,'is_fail'=>0,'is_new'=>1,'is_pay'=>0,'uid' => $uid];
        return ($this->dao->getWhere($where,'*',['productAttr' => function($query) {
            $query->field('unique,score');
        }]));
    }
