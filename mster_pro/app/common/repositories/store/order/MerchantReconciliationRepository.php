...
class MerchantReconciliationRepository extends BaseRepository
{
    ...
    public function create(int $id, array $data)
    {
        ...
        $createData = [
            ...
            'order_score'   => round($compute['order_score'],2),
            'refund_price'  => round($compute['refund_price'],2),
            'refund_score'  => round($compute['refund_score'],2),
    ...
    {
        $order_price = $order_score = $refund_price = $refund_score = $order_extension = $refund_extension = $order_rate = $refund_rate =0;
        $orderMake = app()->make(StoreOrderRepository::class);
        $refundMake = app()->make(StoreRefundOrderRepository::class);
        ...
        foreach($order_ids as $item){
            ...
            //积分
            $order_score = bcadd($order_score,$order['score'],3);
        ...
        foreach($refund_order_ids as $item){
            ...
            //积分
            $refund_score = bcadd($refund_score,$refundOrder['refund_score'],3);
        }

        $price = bcsub($order_price,$refund_price,3);
        $score = bcsub($order_score,$refund_score,3);

        return compact('price','score','refund_price','refund_score','order_extension','refund_extension','order_price','order_score','order_rate');
    }
