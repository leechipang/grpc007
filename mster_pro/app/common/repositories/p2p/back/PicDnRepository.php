...
use app\common\dao\p2p\back\PicDnDao;
...
/**
 * Class PicDnRepository
 * @package app\common\repositories\p2p\back
 */
class PicDnRepository extends BaseRepository
{
    /**
     * PicDnRepository constructor.
     * @param PicDnDao $dao
     */
    public function __construct(PicDnDao $dao)
    {
        $this->dao = $dao;
    }
    ...
    {
        $form = Elm::createForm(is_null($id) ? Route::buildUrl('systemPicDnCreate')->build() : Route::buildUrl('systemPicDnUpdate', ['id' => $id])->build());
        $form->setRule([
            Elm::frameImage('pic', '新品图片（660*180px）', '/' . config('admin.admin_prefix') . '/setting/uploadPic?field=pic&type=2')->width('660px')->height('180px')->props(['footer' => false])->modal(['modal' => false, 'custom-class' => 'suibian-modal']),
...
//            Elm::input('info', '分类简介'),
            Elm::frameImage('pic', '新品图片（660*180px）', '/' . config('admin.admin_prefix') . '/setting/uploadPicture?field=pic&type=1')->width('896px')->height('480px')->props(['footer' => false])->modal(['modal' => false, 'custom-class' => 'suibian-modal']),
