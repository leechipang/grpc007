...
class StoreRefundOrderDao extends BaseDao
{
    ...
    public function search(array $where)
    {
        if(isset($where['is_trader']) && $where['is_trader'] !== ''){
            ...
            $query = (StoreRefundOrder::getDB())->alias('StoreRefundOrder')->join('StoreOrder SO','SO.order_id=StoreRefundOrder.order_id');
        }
        $query->when(isset($where['mer_id']) && $where['mer_id'] !== '', function ($query) use ($where) {
            ...
            $query->where('StoreRefundOrder.order_id','in',$ids);
            ...
            $query->where('StoreRefundOrder.uid', $where['uid']);
            ...
            $query->where('StoreRefundOrder.order_id', $where['order_id']);
        })->field('StoreRefundOrder.*,SO.score')->order('StoreRefundOrder.create_time DESC');
     ...
     * @return mixed
     */
    public function refundScoreByOrder(array $orderIds)
    {
        return $this->getModel()::getDB()->whereIn('order_id',$orderIds)->where('status',3)->sum('refund_score');
    }
