...
//            })
            ->when(isset($where['score']) && isset($where['now_money']),function ($query) use ($where) {
                if ($where['score'] == 0 && $where['now_money'] == 0)
                    $query->whereIn('category', ['score','now_money'])->whereIn('type', ['pay_product', 'recharge', 'sys_inc_money', 'sys_dec_money','sys_inc_score', 'sys_dec_score', 'brokerage', 'presell']);
                else if ($where['score'] == 1 && $where['now_money'] == 1)
                    $query->whereIn('category',  ['score','now_money'])->whereIn('type', ['pay_product', 'sys_dec_score','sys_dec_money', 'presell']);
                else if ($where['score'] == 2 && $where['now_money'] == 2)
                    $query->whereIn('category',  ['score','now_money'])->whereIn('type', ['recharge', 'sys_inc_score', 'sys_inc_money','brokerage']);
            })
    ...
    public function searchJoin(array $where)
    {
        return UserBill::getDB()->alias('a')->leftJoin('User b', 'a.uid = b.uid')
            ->field('a.bill_id,a.pm,a.title,a.number,a.balance,a.numbers,a.balances,a.mark,a.create_time,a.status,b.nickname,a.uid')
