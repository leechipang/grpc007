...
class UserDao extends BaseDao
{
    ...
    public function search(array $where)
    {
        ...
        })->when(isset($where['card_id']) && $where['card_id'] !== '', function (BaseQuery $query) use ($where) {
            return $query->whereLike('User.card_id', "%{$where['card_id']}%");
    ...
    public function userOrderDetail($uid)
    {
        return User::getDB()->alias('A')
            ...
            ->field('A.uid,A.avatar,A.nickname,A.now_money,A.score,A.pay_price,A.pay_count, sum(B.pay_price + IFNULL(C.pay_price,0)) as total_pay_price, count(B.order_id) as total_pay_count')
            ->where('A.uid', $uid)
            ->find();
    }
    ...
    public function checkUser($where,$field){
        return User::getDB()->where($where)->field($field)->find();
    }

    public function saveUser($data){
        return User::getDB()->insertGetId($data);
    }
}
