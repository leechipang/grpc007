...
class User extends BaseController
{
    ...
    public function changeScoreForm($id)
    {
        if (!$this->repository->exists($id))
            return app('json')->fail('数据不存在');
        return app('json')->success(formToData($this->repository->changeScoreForm($id)));
    }
    ...
    public function changeScore($id, UserNowMoneyValidate $validate)
    {
        $data = $this->request->params(['score', 'type']);
        $validate->check($data);
        if (!$this->repository->exists($id))
            return app('json')->fail('数据不存在');
        $this->repository->changeScore($id, $this->request->adminId(), $data['type'], $data['score']);
