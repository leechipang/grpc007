...
class StoreCategory extends BaseController
{
    ...
    public function create(validate $validate)
    {
        ...
        if($data['pid']!=0 && $data['backImg']){
            return app('json')->fail('一级分类才需要设置分类背景图');
        }
    ...
    public function changeStatus($id)
    {
        ...
        if (!$this->repository->merExists($this->request->merId(), $id))
            return app('json')->fail('数据不存在');
        $this->repository->changeStatus($id, ['in_home'=>$status]);
        return app('json')->success('修改成功');
    }
    ...
    {
        $data = $this->request->params(['pid','cate_name','is_show','in_home','pic','backImg','sort']);
        $validate->check($data);
        return $data;
    }
