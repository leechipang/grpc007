...
class StoreSpu extends BaseController
{
    ...
    public function lst()
    {
        ...
        $where = $this->request->params([
            'keyword', 'cate_id', 'order', 'price_on', 'price_off', 'brand_id', 'pid','mer_cate_id','product_type','action','ppid',
            'is_hot','is_benefit','is_best','is_new','is_good'
        ]);
        $where['is_gift_bag'] = 0;
    ...
    public function recommend()
    {
        ...
        $where['is_gift_bag'] = 0;
        $label = $this->request->params(['is_hot','is_benefit','is_best','is_new','is_good']);
        $where += $label;
        $data = $this->repository->getApiSearch($where, $page, $limit, $this->userInfo);
        return app('json')->success($data);
    }
